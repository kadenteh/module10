...
class WMATATest(unittest.TestCase):
    ...
    def test_http_success(self):
        escalator_response = app.test_client().get(
            "/incidents/escalators"
        ).status_code

        # assert that /incidents/escalators returns a 200 code
        self.assertEqual(escalator_response, 200)

        elevator_response = app.test_client().get(
            "/incidents/elevators"
        ).status_code

        # assert that /incidents/elevators returns a 200 code
    ...
    def test_required_fields(self):
        ...
        # for each incident in the JSON response, assert that
    ...
    # ensure all entries returned by the /escalators endpoint
    # have a UnitType of "ESCALATOR"
    def test_escalators(self):
        response = app.test_client().get("/incidents/escalators")
        json_response = json.loads(response.data.decode())

        # for each incident, assert that UnitType is ESCALATOR
        for incident in json_response:
            self.assertEqual(
                incident["UnitType"],
                "ESCALATOR"
    ...
    # ensure all entries returned by the /elevators endpoint
    # have a UnitType of "ELEVATOR"
    def test_elevators(self):
        response = app.test_client().get("/incidents/elevators")
        json_response = json.loads(response.data.decode())

        # for each incident, assert that UnitType is ELEVATOR
        for incident in json_response:
            self.assertEqual(
                incident["UnitType"],
                "ELEVATOR"
